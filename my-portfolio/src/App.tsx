  ...
  return (
    <div className="bg-[#050505] text-white min-h-screen selection:bg-[#d946ef]/30">
      ...
      <div className="fixed top-0 left-0 w-full h-full overflow-hidden -z-10">
        <div className="absolute top-[-5%] left-[-5%] w-[70%] md:w-[45%] h-[45%] bg-[#8b5cf6]/15 blur-[120px] rounded-full" />
        <div className="absolute bottom-[-5%] right-[-5%] w-[70%] md:w-[45%] h-[45%] bg-[#d946ef]/15 blur-[120px] rounded-full" />
      </div>

      <main className="max-w-6xl mx-auto px-6 py-12 md:py-20 space-y-20 md:space-y-32">
        <Hero />

        {/* About Section - Responsive Grid */}
        <section id="about" className="grid lg:grid-cols-2 gap-10 md:gap-16 items-start scroll-mt-24">
          ...
          >
            <div className="flex flex-col md:flex-row gap-6 md:gap-8 items-center md:items-start">
              <div className="relative group shrink-0">
                ...
                <div className="relative w-40 h-40 md:w-48 md:h-48 bg-[#050505] rounded-2xl overflow-hidden border border-white/10">
                  <img 
                    src={diliniProfile} 
                    alt={PROFILE.name}
                ...
                <motion.div
                  ...
                  className="absolute -bottom-2 -right-2 md:-bottom-4 md:-right-4 bg-[#050505] p-2.5 md:p-3 rounded-2xl border border-white/10 shadow-2xl z-20"
                >
                  <div className="relative">
                     <Coffee size={24} className="md:size-[28px] text-[#d946ef]" strokeWidth={1.5} />
                     <motion.div
              ...
              <div className="text-center md:text-left">
                ...
                <p className="text-gray-400 leading-relaxed text-base md:text-lg italic px-2 md:px-0">
                  "{PROFILE.about}"
                </p>
              </div>
            </div>

            <div className="p-6 bg-white/5 border border-white/10 rounded-[2rem] md:rounded-[2.5rem] hover:border-[#8b5cf6]/50 transition-all duration-500 group">
              <h4 className="text-[#8b5cf6] font-bold mb-5 flex items-center gap-2 text-xs uppercase tracking-widest">
                <Code size={18} /> Technical Expertise
              </h4>
              <div className="flex flex-wrap gap-2">
                {PROFILE.techSkills.map((s) => (
                  <span key={s} className="bg-white/5 px-3 py-1.5 rounded-xl text-[10px] md:text-xs text-gray-400 border border-white/5 hover:border-[#d946ef]/50 hover:text-white hover:bg-white/10 transition-all cursor-default">
                    {s}
                  </span>
                ))}
              </div>
          ...
          <motion.div 
            ...
            whileInView={{ opacity: 1, x: 0 }}
            viewport={{ once: true }}
            className="bg-white/5 border border-white/10 p-6 md:p-8 rounded-[2rem] md:rounded-[2.5rem] relative overflow-hidden h-full"
          >
            ...
            <div className="space-y-8 md:space-y-10 relative before:absolute before:left-[11px] before:top-2 before:bottom-2 before:w-[1px] before:bg-gradient-to-b before:from-[#d946ef] before:via-[#8b5cf6] before:to-transparent text-left">
              <div className="relative pl-10 group">
                <div className="absolute left-0 top-1.5 w-[22px] h-[22px] bg-[#050505] border-2 border-[#d946ef] rounded-full" />
                <h5 className="text-white font-bold text-base md:text-lg leading-tight">Diploma in Computer Science</h5>
                <p className="text-[#d946ef] text-xs md:text-sm mt-1">IJSE - Institute of Software Engineering</p>
              </div>
              <div className="relative pl-10 group">
                <div className="absolute left-0 top-1.5 w-[22px] h-[22px] bg-[#050505] border-2 border-[#8b5cf6] rounded-full" />
                <h5 className="text-white font-bold text-base md:text-lg leading-tight">AI & Machine Learning</h5>
                <p className="text-[#8b5cf6] text-xs md:text-sm mt-1">IJSE - Institute of Software Engineering</p>
              </div>
              <div className="relative pl-10 group">
                <div className="absolute left-[4px] top-1.5 w-[14px] h-[14px] bg-gray-700 rounded-full" />
                <h5 className="text-gray-200 font-semibold leading-tight text-base">G.C.E. A/L Physical Science (CCS)</h5>
                <p className="text-gray-500 text-xs mt-1">St. John's College</p>
              </div>
            </div>
          </motion.div>
        </section>
